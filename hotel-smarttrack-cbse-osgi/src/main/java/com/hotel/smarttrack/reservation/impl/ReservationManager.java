...
public class ReservationManager implements ReservationService {
    ...
    public void activate() {
        ...
        // seed 1 safe reservation (only using known setters from Reservation entity)
    ...
    public Reservation createReservation(Long guestId, Long roomTypeId, LocalDate checkIn,
        ...
        // NOTE: to avoid cross-bundle dependencies, we do not resolve Guest/RoomType here.
        // guestId/roomTypeId can be validated later when those services are integrated.
    ...
    public List<Reservation> getReservationsByGuest(Long guestId) {
        // No Guest linkage stored (avoid dependency). Return empty for now.
        return List.of();
    }
    ...
    public void assignRoom(Long reservationId, Long roomId) {
        ...
        // store only roomId mapping to avoid touching Room entity / other bundles
        repo.assignRoom(reservationId, roomId);

        // keep status meaningful
        if (r.getStatus() == null || "Reserved".equalsIgnoreCase(r.getStatus())) {
            r.setStatus("Confirmed");
            repo.save(r);
        }
    }
    ...
    public List<Long> searchAvailableRooms(LocalDate checkIn, LocalDate checkOut, Long roomTypeId, int occupancy) {
        // To avoid dependency on RoomService bundle, keep as stub.
        return List.of();
    }
